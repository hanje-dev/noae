...
import immer from '@noaejs/deps/compiled/immer';
import * as fs from 'fs';
import { join } from 'path';
import { IHttps, IServerOpts } from './Server';

const logger = new Logger('@noaejs/server:utils');
...
export const getCredentials = (opts: IServerOpts): IHttps => {
  ...
  const serverOptions = (https === true ? defautlServerOptions : https) as IHttps;
