import { Logger } from '@noaejs/core';
import compress, { CompressionOptions } from '@noaejs/deps/compiled/compression';
import express, { Express, RequestHandler } from '@noaejs/deps/compiled/express';
import {
  createProxyMiddleware,
  Filter as ProxyFilter,
  Options as ProxyOptions,
  RequestHandler as ProxyRequestHandler,
} from '@noaejs/deps/compiled/http-proxy-middleware';
import sockjs, { Connection, Server as SocketServer } from '@noaejs/deps/compiled/sockjs';
import spdy, { ServerOptions } from '@noaejs/deps/compiled/spdy';
import { createDebug, lodash, PartialProps, portfinder } from '@noaejs/utils';
import * as http from 'http';
import * as url from 'url';
import { getCredentials } from './utils';

const logger = new Logger('@noaejs/server');
const debug = createDebug('noae:server:Server');
...
class Server {
   ...
   */
  setupCompress() {
    const compressOpts = lodash.isBoolean(this.opts.compress) ? {} : this.opts.compress;
  ...
  async listen({ port = 8000, hostname }: { port?: number; hostname: string }): Promise<{
    port: number;
    hostname: string;
    listeningApp: http.Server;
    server: Server;
