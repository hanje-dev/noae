...
export function getBundleOpts(opts: IOpts): IBundleOptions[] {
  ...
  return userConfigs.map((config: IBundleOptions) => {
  ...
  if (!bundleOpts.esm && !bundleOpts.cjs && !bundleOpts.umd) {
    throw new Error(
      `None format of ${chalk.cyan(
        'cjs | esm | umd'
      )} is configured, checkout https://github.com/umijs/father for usage details. `.trim()
    );
  }
  if (bundleOpts.entry) {
    const tsConfigPath = join(cwd, 'tsconfig.json');
    const tsConfig = existsSync(
      tsConfigPath || (rootPath ?? existsSync(join(rootPath, 'tsconfig.json')))
    );
    if (
      tsConfig &&
...
export async function buildForLerna(opts: IOpts) {
  ...
  if (userConfig.pkgs) {
    pkgs = userConfig.pkgs
      .map((item) => pkgs.find((pkg) => basename(pkg.contents) === item))
      .filter(Boolean);
  }
