...
const { parser, traverse, winPath } = utils;
export type ModelItem = { absPath: string; namespace: string; exportName?: string } | string;
...
export const genImports = (imports: string[]) =>
  imports.map((ele, index) => `import model${index} from "${winPath(getPath(ele))}";`).join(EOL);
...
export const genModels = (imports: string[], absSrcPath: string) => {
    ...
    traverse.default(ast, {
      enter(astPath) {
        if (astPath.isIdentifier({ name: 'useModel' })) {
          try {
            // string literal
            const ns = ((astPath.parentPath as any).node as any).arguments[0].value;
  ...
  if (checkDuplicates(contents.map((ele) => ele.namespace))) {
    throw Error('noae: models 中包含重复的 namespace！');
  }
...
export const isValidHook = (filePath: string) => {
  ...
  try {
    if (identifierName) {
      ast.program.body.forEach((ele: any) => {
        ...
        if (ele.type === 'VariableDeclaration') {
          if (
            (ele.declarations[0].id as utils.t.Identifier).name === identifierName &&
            (ele.declarations[0].init as utils.t.ArrowFunctionExpression).type ===
