...
import { t, winPath } from '@noaejs/utils';
...
export default function () {
  return {
    visitor: {
          ...
          while (index >= 0) {
              ...
              if (
                ...
                const init = t.awaitExpression(
                  t.callExpression(t.import(), [
                    t.stringLiteral(
                      isMatch
                        ? `${opts.remoteName}/${getPath(d.source.value, opts.alias || {})}`
                        : d.source.value
                    ),
                  ])
                );
            ...
            if (t.isExportAllDeclaration(d) && d.source) {
              ...
              opts.onTransformDeps?.({
                ...
                // @ts-ignore
                file: path.hub.file.opts.filename,
                isMatch: isMatch && opts.exportAllMembers?.[d.source.value]?.length,
            ...
            if (t.isExportNamedDeclaration(d) && d.source) {
              ...
              if (isMatch) {
                const { properties, defaultIdentifier } = specifiersToProperties(d.specifiers);
                const id = t.objectPattern(properties);
                const init = t.awaitExpression(
                  t.callExpression(t.import(), [
                    t.stringLiteral(
      ...
      CallExpression(path: NodePath<t.CallExpression>, { opts }: { opts: IOpts }) {
        const { node } = path;
        if (
