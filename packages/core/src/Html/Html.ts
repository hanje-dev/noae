import ejs from '@noaejs/deps/compiled/ejs';
import prettier from '@noaejs/deps/reexported/prettier';
import { cheerio } from '@noaejs/utils';
...
class Html {
  ...
  getRelPathToPublicPath(path: string) {
    ...
    return (
      Array(this.config.exportStatic && this.config.exportStatic.htmlSuffix ? len : len - 1).join(
        '../'
      ) || './'
    );
  }
