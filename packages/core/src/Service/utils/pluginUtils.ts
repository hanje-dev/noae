import { compatESModuleRequire, createDebug, lodash, pkgUp, resolve, winPath } from '@noaejs/utils';
...
const debug = createDebug('noae:core:Service:util:plugin');
...
const RE = {
  [PluginType.plugin]: /^(@noaejs\/|noae-)plugin-/,
  [PluginType.preset]: /^(@noaejs\/|noae-)preset-/,
};
...
function getPluginsOrPresets(type: PluginType, opts: IOpts): string[] {
  ...
  return [
    ...
    // env
    ...(process.env[`NOAE_${upperCaseType}S`] || '').split(',').filter(Boolean),
    // dependencies
...
function pkgNameToKey(pkgName: string, type: PluginType) {
  // strip none @noaejs scope
  if (pkgName.charAt(0) === '@' && !pkgName.startsWith('@noaejs/')) {
...
export function pathToObj({ type, path, cwd }: { type: PluginType; path: string; cwd: string }) {
  ...
  id = id.replace('@noaejs/preset-built-in/lib/plugins', '@@');
  id = id.replace(/\.js$/, '');
