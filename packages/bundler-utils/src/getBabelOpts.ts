...
function getBasicBabelLoaderOpts({ cwd }: { cwd: string }) {
  ...
  return {
    ...
    babelrc: false,
    cacheDirectory:
      process.env.BABEL_CACHE !== 'none' ? winPath(`${prefix}/.noae/.cache/babel-loader`) : false,
  };
}
...
export function getBabelOpts({
  ...
  return {
    ...
    presets: [
      [require.resolve('@noaejs/babel-preset-noae/app'), presetOpts],
...
export function getBabelDepsOpts({ env, cwd, config }: { env: env; cwd: string; config: IConfig }) {
  return {
    ...getBasicBabelLoaderOpts({ cwd }),
    presets: [
      [
        require.resolve('@noaejs/babel-preset-noae/dependency'),
        {
          nodeEnv: env,
          dynamicImportNode: !config.dynamicImport && !config.dynamicImportSyntax,
        },
