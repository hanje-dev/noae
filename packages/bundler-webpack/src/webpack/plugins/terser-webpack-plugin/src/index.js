...
import { SourceMapConsumer } from '@noaejs/deps/compiled/source-map';
import webpack, {
  util,
...
import pLimit from '@noaejs/deps/compiled/p-limit';
...
const { SourceMapSource, RawSource, ConcatSource } =
  ...
  webpack.sources || require('@noaejs/deps/compiled/webpack-sources');
...
class TerserPlugin {
  ...
  static buildError(error, file, sourceMap, requestShortener) {
    if (error.line) {
      ...
      if (original && original.source && requestShortener) {
        return new Error(
          `${file} from Terser\n${error.message} [${requestShortener.shorten(original.source)}:${
            original.line
  ...
  async optimize(compiler, compilation, assets, CacheEngine, weakCache) {
    ...
    if (TerserPlugin.isWebpack4()) {
      assetNames = []
        ...
        .concat(
          ...
          Array.from(assets).reduce((acc, chunk) => acc.concat(Array.from(chunk.files || [])), [])
        )
        .concat(Object.keys(compilation.assets))
        .filter((assetName, index, existingAssets) => existingAssets.indexOf(assetName) === index)
        .filter((assetName) =>
    ...
    const cache = new CacheEngine(compilation, { cache: this.options.cache }, weakCache);
    ...
    for (const name of assetNames) {
      scheduledTasks.push(
        limit(async () => {
          const { info, source: inputSource } = TerserPlugin.getAsset(compilation, name);
      ...
      typeof this.options.sourceMap === 'undefined'
        ? (devtool &&
          ...
          (plugins &&
            plugins.some(
              (plugin) =>
                plugin instanceof SourceMapDevToolPlugin && plugin.options && plugin.options.columns
            ))
