import { BundlerConfigType, IBundlerConfigType, IConfig } from '@noaejs/types';
...
export function createCSSRule({
  ...
  function applyLoaders(rule: Config.Rule<Config.Rule>, isCSSModules: boolean) {
    if (config.styleLoader) {
      rule
        .use('style-loader')
        .loader(require.resolve('@noaejs/deps/compiled/style-loader'))
        .options(
          deepmerge(
            {
              base: 0,
    ...
    if (isDev && isCSSModules && config.cssModulesTypescriptLoader) {
      rule
        ...
        .loader(require.resolve('@noaejs/deps/compiled/css-modules-typescript-loader'))
        .options(config.cssModulesTypescriptLoader);
    }

    rule
      .use('css-loader')
      .loader(require.resolve('@noaejs/deps/compiled/css-loader'))
      .options(
        deepmerge(
          {
            importLoaders: 1,
...
export default function ({
  ...
  createCSSRule({
    ...
    lang: 'less',
    test: /\.(less)(\?.*)?$/,
    loader: require.resolve('@noaejs/deps/compiled/less-loader'),
    options: deepmerge(
      {
        modifyVars: theme,
  ...
  if (!config.styleLoader) {
    ...
    if (type === BundlerConfigType.csr) {
      webpackConfig
        .plugin('extract-css')
        .use(
          miniCSSExtractPluginPath || require.resolve('../webpack/plugins/mini-css-extract-plugin'),
          [
            {
  ...
  if (!isDev && !disableCompress) {
    webpackConfig
      .plugin('optimize-css')
      .use(require.resolve('@noaejs/deps/compiled/optimize-css-assets-webpack-plugin'), [
        {
          cssProcessorOptions: {
