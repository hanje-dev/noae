...
export function init() {
  ...
  const filesMap = files.map((file) => {
    ...
    return [file, `@noaejs/deps/compiled/webpack/${fileName}`];
  });

  const hookPropertyMap = new Map(
    [
      ['webpack', '@noaejs/deps/compiled/webpack'],
      ['webpack/package.json', '@noaejs/deps/compiled/webpack/pkgInfo'],
      ...filesMap,
      // ['webpack-sources', '@noaejs/deps/compiled/webpack/sources'],
