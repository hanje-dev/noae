import { escapeRegExp } from '@noaejs/deps/compiled/lodash';
 ...
 * @param param0
 */
export const routeToChunkName: IRouteToChunkName = ({ route, cwd } = { route: {} }) => {
  return typeof route.component === 'string'
    ? route.component
        .replace(new RegExp(`^${escapeRegExp(lastSlash(winPath(cwd || '/')))}`), '')
        .replace(/^.(\/|\\)/, '')
        .replace(/(\/|\\)/g, '__')
        .replace(/\.jsx?$/, '')
        .replace(/\.tsx?$/, '')
