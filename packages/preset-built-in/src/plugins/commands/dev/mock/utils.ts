import bodyParser from '@noaejs/deps/compiled/body-parser';
import multer from '@noaejs/deps/compiled/multer';
import pathToRegexp from '@noaejs/deps/compiled/path-to-regexp';
import { IApi, IRoute, NextFunction, Request, RequestHandler } from '@noaejs/types';
import { createDebug, glob, winPath } from '@noaejs/utils';
...
const debug = createDebug('noae:preset-build-in:mock:utils');
...
export const getConflictPaths = ({
  ...
  mockData: IMockDataItem[];
  routes: IRoute[];
}): Array<Pick<IMockDataItem, 'path'>> => {
  const conflictPaths: Array<Pick<IMockDataItem, 'path'>> = [];
