...
import { delay } from '@noaejs/utils';
...
export default (api: IApi) => {
  ...
  api.registerCommand({
    ...
    async fn({ args }) {
      ...
      server = new Server({
        ...
        beforeMiddlewares,
        afterMiddlewares: [...middlewares, createRouteMiddleware({ api, sharedMap })],
  ...
  api.registerMethod({
    name: 'getServer',
    fn() {
      assert(api.env === 'development', `api.getServer() is only valid in development.`);
      return server;
    },
  });
