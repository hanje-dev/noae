  ...
  api.registerCommand({
    ...
    async fn() {
      ...
      let config = bundleConfigs.filter((bundleConfig: any) => {
        return bundleConfig.entry?.noae;
      })[0];
      assert(config, `No valid config found with noae entry.`);

      if (api.args.rule) {
        config = config.module.rules.find((r: any) => r.__ruleNames[0] === api.args.rule);
      } else if (api.args.plugin) {
        config = config.plugins.find((p: any) => p.__pluginName === api.args.plugin);
