import { Bundler as DefaultBundler, webpack } from '@noaejs/bundler-webpack';
import { BundlerConfigType, IApi, IBundlerConfigType } from '@noaejs/types';
import { chalk, rimraf } from '@noaejs/utils';
  ...
  async function getConfig({ type }: { type: IBundlerConfigType }) {
    ...
    const getConfigOpts = await api.applyPlugins({
      ...
      initialValue: {
        ...
        entry: {
          noae: join(api.paths.absTmpPath!, 'noae.ts'),
        },
        // @ts-ignore
        bundleImplementor,
...
export function cleanTmpPathExceptCache({ absTmpPath }: { absTmpPath: string }) {
...
export function printFileSizes(stats: webpack.Stats, dir: string) {
  const ui = require('@noaejs/deps/compiled/cliui')({ width: 80 });
  ...
  const orderedAssets = assets
    ?.map((a) => {
      ...
      const isMainBundle = a.name.indexOf('noae.') === 0;
