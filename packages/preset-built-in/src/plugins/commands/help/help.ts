import { ICommand } from '@noaejs/core/lib/Service/types';
import { IApi } from '@noaejs/types';
import { chalk, lodash } from '@noaejs/utils';
  ...
  api.registerCommand({
    ...
    fn({ args }) {
      ...
      if (commandName) {
        ...
        console.log(`
  Usage: noae ${commandName} [options]
...
${command.details ? padLeft(command.details.trim()) : ''}
        ...
        console.log(`
  Usage: noae <command> [options]

  Commands:

${getDescriptions(api.service.commands).join('\n')}

  Run \`${chalk.bold('noae help <command>')}\` for more information of specific commands.
  Visit ${chalk.bold('https://umijs.org/')} to learn more about Noae.
      `);
      }
    },
  });
