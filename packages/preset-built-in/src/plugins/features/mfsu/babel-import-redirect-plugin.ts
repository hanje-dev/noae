...
import { lodash, t } from '@noaejs/utils';
...
export default function () {
  return {
    visitor: {
      ImportDeclaration: {
        exit(path: NodePath<t.Program>, { opts: redirect }: { opts: IRedirect }) {
          ...
          if (!lodash.intersection(imports, Object.keys(redirect[value])).length) {
            return;
          }
          ...
          if (restImport.length) {
            path.replaceWith(t.importDeclaration(restImport, t.stringLiteral(value)));
          } else {
            path.remove();
          }
        },
