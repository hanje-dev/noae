import { IApi, IConfig } from '@noaejs/types';
...
const debug = createDebug('noae:mfsu:DepInfo');
...
export default class DepInfo {
  ...
  constructor(opts: { tmpDir: string; api: IApi; cwd: string; mode: TMode; webpackAlias: any }) {
    this.api = opts.api;
    this.cwd = opts.cwd;
  ...
  setTmpDep(opts: { key: string; version: string; from: string }) {
    if (this.data.tmpDeps[opts.key] && this.data.tmpDeps[opts.key] !== opts.version) {
      throw new Error(
        `[MFSU] dep ${opts.key} conflicts of ${opts.version} and ${this.data.tmpDeps[opts.key]}`
      );
    }
  ...
  shouldBuild(): boolean {
    ...
    if (!lodash.isEqual(this.getConfig(), this.data.config)) {
      debug(
        `config changed, new: ${JSON.stringify(this.getConfig())}, origin: ${JSON.stringify(
          this.data.config
        )}`
      );
      return true;
  ...
  getConfig() {
    return {
      ...
      noaeVersion: process.env.NOAE_VERSION,
