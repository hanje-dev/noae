...
import { BundlerConfigType, IApi } from '@noaejs/types';
import { chalk, createDebug, mkdirp } from '@noaejs/utils';
...
import { getNoaeRedirect } from './getNoaeRedirect';
import { copy } from './utils';

const debug = createDebug('noae:mfsu');
...
export const getMfsuPath = (api: IApi, { mode }: { mode: TMode }) => {
  if (mode === 'development') {
    ...
    return configPath ? join(api.cwd, configPath) : join(api.paths.absTmpPath!, '.cache', '.mfsu');
  }
  const configPath = api.userConfig.mfsu?.production?.output;
  return configPath ? join(api.cwd, configPath) : join(api.cwd, './.mfsu-production');
};

export const isMonacoWorker = (api: IApi, reqPath: string, fileRelativePath: string) =>
...
export default function (api: IApi) {
        ...
        ...(args.mfsu
          ...
          : {
              importToAwaitRequire: {
                remoteName: (api.config.mfsu && api.config.mfsu.mfName) || DEFAULT_MF_NAME,
                matchAll: true,
                webpackAlias,
                webpackExternals,
                alias: {
  ...
  api.modifyBabelOpts({
    fn: async (opts) => {
      webpackAlias['core-js'] = dirname(require.resolve('core-js/package.json'));
      webpackAlias['regenerator-runtime/runtime'] = require.resolve('regenerator-runtime/runtime');

      // @ts-ignore
      const noaeRedirect = await getNoaeRedirect(process.env.NOAE_DIR);

      // 降低 babel-preset-noae 的优先级，保证 core-js 可以被插件及时编译
      opts.presets?.forEach((preset) => {
        if (preset instanceof Array && /babel-preset-noae/.test(preset[0])) {
    ...
    return (req, res, next) => {
      ...
      if (isMfAssets) {
        depBuilder.onBuildComplete(() => {
          ...
          res.setHeader('content-type', `${mime.lookup(parse(path || '').ext)}; charset=UTF-8`);
  ...
  api.register({
    ...
    fn(memo: any, { type, mfsu }: { mfsu: boolean; type: BundlerConfigType }) {
      if (type === BundlerConfigType.csr) {
        ...
        if (!mfsu) {
          const mfName = (api.config.mfsu && api.config.mfsu.mfName) || DEFAULT_MF_NAME;
          memo.plugins.push(
            ...
            memo.plugins.push(
              new (class MonacoEditorWebpackPluginHack {
                apply(compiler: webpack.Compiler) {
                  const { taps } = compiler.hooks.make;
  ...
  async function buildDeps(opts: { force?: boolean } = {}) {
    ...
    if (mode === 'production') {
      ...
      copy(depBuilder.tmpDir, join(api.cwd, api.userConfig.outputPath || './dist'));
    }
  }

  // npx noae mfsu build
  // npx noae mfsu build --mode production
  // npx noae mfsu build --mode development --force
  api.registerCommand({
    name: 'mfsu',
    async fn({ args }) {
      switch (args._[0]) {
