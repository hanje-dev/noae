...
import { transform } from '@noaejs/deps/reexported/esbuild';
...
export async function getDepReExportContent(opts: {
  ...
  if (
    opts.filePath &&
    /\.(json|svg|png|jpe?g|gif|webp|ico|eot|woff|woff2|ttf|txt|text|md)$/.test(opts.filePath)
  ) {
    return `
...
export default _;`.trim();
  ...
  try {
    ...
    const { exports, isCJS } = await parseWithCJSSupport(opts.content, opts.filePath);
    // cjs
    if (isCJS) {
      return [
        `import _ from '${opts.importFrom}';`,
...
export const cjsModeEsmParser = (code: string) => {
  return matchAll(/Object\.defineProperty\(\s*exports\s*\,\s*[\"|\'](\w+)[\"|\']/g, code)
    .concat(
    ...
    .concat(
      ...
      ...matchAll(/__webpack_require__\.d\(\s*__webpack_exports__\s*,\s*(\{)/g, code).map(
        (matchResult) => {
          ...
          const result = code.slice(symbolBeginIndex, idx);

          return [...matchAll(/(?:\"|\')(\w+)(?:\"|\')\s*\:\s*(?:function|\()/g, result)];
        }
      )
    )
    .map((result) => result[1]);
...
function hasNonDefaultExports(exports: any) {
  return exports.filter((exp: string) => !['__esModule', 'default'].includes(exp)).length > 0;
}
