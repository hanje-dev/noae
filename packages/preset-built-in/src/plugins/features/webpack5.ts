...
export default (api: IApi) => {
  ...
  api.modifyBundleConfig((memo) => {
    ...
    if (api.env === 'development' && api.config.webpack5?.lazyCompilation) {
      ...
      memo.experiments = {
        ...
        lazyCompilation: {
          ...
          //   '@noaejs/deps/compiled/webpack/5/lazy-compilation-web.js',
          // ),
          backend: require('@noaejs/deps/compiled/webpack/5/lazyCompilationBackend.js'),
          entries: false,
          // @ts-ignore
    ...
    if (process.env.WEBPACK_FS_CACHE !== 'none') {
      ...
      memo.cache = {
        type: 'filesystem',
        // using noae version as `cache.version`
        version: process.env.NOAE_VERSION,
        buildDependencies: {
          config: [
            join(api.cwd, 'package.json'),
            api.config.webpack5 && configFile ? join(api.cwd, configFile) : undefined,
          ].filter(Boolean),
        },
