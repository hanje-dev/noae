...
import { IApi, IRoute } from '@noaejs/types';
import { deepmerge, rimraf } from '@noaejs/utils';
import { existsSync } from 'fs';
import { join } from 'path';
import { Stream } from 'stream';
import { OUTPUT_SERVER_FILENAME } from './ssr/constants';
...
export default (api: IApi) => {
  api.describe({
    ...
    enableBy: () =>
      ...
      'exportStatic' in api.userConfig ? api.userConfig.exportStatic : api.config?.exportStatic,
  });
  ...
  api.onBuildComplete(({ err }) => {
    if (!err && api.config?.ssr) {
      ...
      if (
        process.env.RM_SERVER_FILE ? process.env.RM_SERVER_FILE !== 'none' : !serverRenderFailed
      ) {
        // remove noae.server.js
        const serverFilePath = join(api.paths.absOutputPath!, OUTPUT_SERVER_FILENAME);
