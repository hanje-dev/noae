import { webpack } from '@noaejs/bundler-webpack';
import { Route } from '@noaejs/core';
import serialize from '@noaejs/deps/compiled/serialize-javascript';
// @ts-ignore
import { getCompilerHooks } from '@noaejs/deps/compiled/webpack-manifest-plugin';
import { BundlerConfigType, IApi } from '@noaejs/types';
import { cleanRequireCache, lodash as _, Mustache, routeToChunkName, winPath } from '@noaejs/utils';
...
class ManifestChunksMapPlugin {
  ...
  apply(compiler: webpack.Compiler) {
    ...
    compiler.hooks.emit.tapPromise('ManifestChunksMapPlugin', async (compilation: any) => {
    ...
    beforeEmit.tap('ManifestChunksMapPlugin', (manifest: object) => {
      if (chunkGroups) {
        ...
        const addPath = (file: string) => `${this.opts.api.config.publicPath}${file}`;
        try {
...
export const onBuildComplete =
  ...
  async ({ err, stats }: any) => {
    if (!err && stats?.stats) {
      ...
      // replace `noae.server.js` default html into latest serialize html
      if (fs.existsSync(serverPath) && latestHTML) {
        const serverContent = (await fs.promises.readFile(serverPath, 'utf-8')).replace(
          HTML_REG,
          htmlContent
        );
...
export default (api: IApi) => {
      ...
      schema: (joi) => {
        return joi
          .object({
            ...
            devServerRender: joi
              .boolean()
              .description('disable serve-side render in noae dev mode.'),
  ...
  api.onStart(() => {
    ...
    if (!process.env.WATCH_IGNORED) {
      ...
      const absOutputPath = winPath(path.join(api.cwd, outputPath as string, '/'));
  ...
  api.onGenerateFiles(async () => {
    ...
    api.writeTmpFile({
      ...
      content: Mustache.render(serverContent, {
        ...
        RuntimePath: winPath(path.dirname(require.resolve('@noaejs/runtime/package.json'))),
        ...
        ManifestFileName: api.config.manifest ? api.config.manifest.fileName || CHUNK_MANIFEST : '',
    ...
    api.writeTmpFile({
      ...
      content: Mustache.render(clientExportsContent, {
        SSRUtils: winPath(require.resolve('@noaejs/utils/lib/ssr')),
      }),
    });
  });
  ...
  api.modifyConfig((config) => {
    ...
    config.devServer.writeToDisk = (filePath: string) => {
      const regexp = new RegExp(`(${OUTPUT_SERVER_FILENAME}|${OUTPUT_SERVER_TYPE_FILENAME})$`);
  ...
  api.chainWebpack(async (config, opts) => {
    ...
    if (type === BundlerConfigType.ssr) {
        ...
        [
          {
            ...
            content: `import { IServerRender } from 'noae';${EOL}export = render;${EOL}export as namespace render;${EOL}declare const render: IServerRender;`,
          },
        ],
      ]);
      ...
      config.plugin('ManifestChunksMap').use(ManifestChunksMapPlugin, [{ api }]);
