...
import mergeStream from '@noaejs/deps/compiled/merge-stream';
...
export { default as cheerio } from '@noaejs/utils/lib/cheerio/cheerio';
 ...
 * @param param
 */
export const handleHTML = async (
  opts: Partial<IHandleHTMLOpts> = {}
  ...
  const windowInitialVars = {
    ...(pageInitialProps && !removeWindowInitialProps
      ? {
          'window.g_initialProps': serialize(forceInitial ? null : pageInitialProps),
        }
      : {}),
  };
      ...
      chunks.forEach((chunk) => {
        ...
        assets[chunk].forEach((resource: string) => {
          if (/\.css$/.test(resource))
            cssChunkSet.push(
              `<link rel="preload" href="${resource}" as="style" /><link rel="stylesheet" href="${resource}" />`
            );
        });
      });
      // avoid repeat
