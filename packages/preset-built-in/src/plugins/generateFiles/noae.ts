...
import { resolve, winPath } from '@noaejs/utils';
import { readFileSync } from 'fs';
import { join } from 'path';
import { renderReactPath, runtimePath } from './constants';

export function importsToStr(imports: Array<{ source: string; specifier?: string }>) {
...
function isReact18(opts: { pkg: any; cwd: string }) {
  ...
  function getVersion(name: string) {
    try {
      ...
      return parseInt(JSON.parse(readFileSync(pkgJSONPath, 'utf-8')).version.split('.')[0], 10);
    } catch (e) {
      return 0;
    }
  }
...
export default function (api: IApi) {
  ...
  api.onGenerateFiles(async () => {
    const noaeTpl = readFileSync(join(__dirname, 'noae.tpl'), 'utf-8');
    ...
    api.writeTmpFile({
      path: 'noae.ts',
      content: Mustache.render(noaeTpl, {
        // @ts-ignore
