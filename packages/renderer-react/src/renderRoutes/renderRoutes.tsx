import { ApplyPluginsType, Plugin, Redirect } from '@noaejs/runtime';
import React, { createElement, useEffect, useState } from 'react';
import { IComponent, IRoute } from '../index';
    ...
    useEffect(() => {
      ...
      const handleGetInitialProps = async () => {
        ...
        if (preloadComponent?.getInitialProps) {
          ...
          const initialProps = await preloadComponent!.getInitialProps!(ctx || defaultCtx);
...
function getRouteElement({ route, index, opts }: IGetRouteElementOpts) {
  ...
  if (route.redirect) {
    ...
    if (
      ...
      // TODO need a type
      ((route.component as any)?.getInitialProps || (route.component as any)?.preload)
    ) {
